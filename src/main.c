...
volatile uint16_t current_frequency = FREQ_MIN_HZ;
...
static void Update_Frequency(uint16_t frequency);
static uint16_t Map_ADC_to_Frequency(uint16_t adc_val);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
    {
      ...
      uint16_t new_frequency = Map_ADC_to_Frequency(adc_value);
...
static uint16_t Map_ADC_to_Frequency(uint16_t adc_val)
{
  // Map ADC value (0-4095) to frequency range (10Hz-1000Hz)
